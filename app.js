...
async function renderApplication() {
  ...
  renderInFlight = (async () => {
    try {
      ...
      if (!seller) throw new Error("Seller profile was not found. Run the Seller/Shop database setup first.");
      ...
      if (currentRoute === "workflow") {
        if (!workflowProductId) { navigate("products"); return; }
        showScreen("workflow");
        await loadWorkflow();
        return;
      }
...
async function loadProducts() {
  ...
  const { data, error } = await supabase.from("products")
    .select(`id,seller_id,name,default_price,customer_cancellable_until_stage,is_active,created_at,updated_at`)
  ...
  if (!data || data.length === 0) $("emptyProductsState").hidden = false;
}
...
async function loadQrSeries() {
  ...
  const { data, error } = await supabase.from("qr_codes")
    .select(`id,product_id,series_name,series_sequence,code,status,created_at,products(name)`)
    .eq("seller_id", user.id)
  ...
  (data || []).forEach(qr => {
    ...
    g.total += 1;
    if (qr.status === "available") g.available += 1;
    else if (qr.status === "assigned") g.assigned += 1;
    else if (qr.status === "revoked") g.revoked += 1;
  });
