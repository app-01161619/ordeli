-- Harden direct payment writes.
-- Sellers must use the guarded RPCs for payment creation/review/method updates.
...
begin
  ...
  where id = p_payment_id and seller_id = v_user
  for update;
  if not found then
  ...
  where id = v_payment.id and seller_id = v_user;
