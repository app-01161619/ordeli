-- Ordeli: stable offline-order synchronization RPC.
-- This intentionally uses a new RPC name so older overloaded sync_offline_order
-- functions cannot interfere with PostgREST schema resolution.
...
begin
  ...
  -- Idempotency: a retry of a completed queue row returns the original order.
  ...
  -- Lock the QR row so two online/offline replays cannot assign it at once.
  select * into v_qr
  ...
  if v_qr.order_item_id is not null
     or lower(coalesce(v_qr.status, '')) <> 'available' then
    raise exception 'This QR code is no longer available.';
  end if;

  -- An offline-created transaction may only consume a QR reserved to this
  -- seller/device. This is what prevents duplicate QR use across devices.
  if not exists (
    select 1
  ...
  on conflict (seller_id, client_order_id)
  do update set
...
grant execute on function public.sync_offline_order_v3(text, text, text, uuid, text, text, integer, numeric)
to authenticated;
