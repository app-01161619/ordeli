-- Ordeli security hardening
-- 1) A revoked QR must never authorize customer production-proof lookup.
-- 2) SECURITY DEFINER proof lookup remains explicitly callable only by the
--    public customer roles; the function itself performs the token check.
...
revoke all on function public.get_customer_stage_proof_v2(text, integer) from public, anon, authenticated;
