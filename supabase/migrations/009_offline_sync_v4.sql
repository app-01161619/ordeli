-- Ordeli: single, unambiguous offline order sync RPC.
-- Built for the deployed schema represented by the current database structure:
-- orders.event_id / pickup_status; qr_codes.order_item_id; offline_order_syncs.
-- This intentionally uses a new RPC name so prior overloaded functions cannot collide.

begin;

drop function if exists public.sync_offline_order_v4(text,text,text,uuid,text,text,integer,numeric);

create function public.sync_offline_order_v4(
...
declare
  ...
  v_downpayment numeric(12,2) := greatest(coalesce(p_downpayment,0),0);
begin
  ...
  -- Idempotency: a retry returns the original server order.
  select * into v_sync
  ...
  if found and v_sync.order_id is not null then
    ...
    from public.orders
    where id = v_sync.order_id and seller_id = v_seller_id;
  ...
  -- Lock and validate the QR. Offline assignment requires the reservation
  -- to belong to this seller/device, preventing cross-device collisions.
  select * into v_qr
  ...
  if v_qr.status <> 'available' or v_qr.order_item_id is not null then
  ...
  if p_customer_id is not null then
    ...
    where id = p_customer_id and seller_id = v_seller_id;
    if not found then
  ...
  v_total := (coalesce(v_product.default_price,0) * p_quantity)::numeric(12,2);
  if v_downpayment > v_total then
    raise exception 'Downpayment cannot exceed the order total.';
  end if;

  insert into public.orders (
    seller_id, customer_id, fulfillment_type, event_id, pickup_status,
    created_at, updated_at
  ) values (
    v_seller_id, v_customer_id, null, null, 'not_scheduled', now(), now()
  )
