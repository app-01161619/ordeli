-- Ordeli: canonical offline order synchronization RPC.
-- Replays a locally created order exactly once per seller/device/client_order_id.
-- Uses the existing offline_order_syncs table for idempotency.
...
begin
  ...
  -- Idempotency: retries/reloads return the original server order.
  ...
  -- Offline assignment is only valid for the device that reserved the QR.
  -- A seller may have multiple device reservations, but this replay must match.
