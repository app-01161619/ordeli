-- Ordeli: canonical signature for adding an item to an existing order online.
-- PostgREST resolves RPCs by their argument names/types, so keep this signature
-- aligned with the client and with the schema cache:
--   p_device_id, p_order_id, p_qr_public_token, p_quantity

begin;

-- Remove the earlier argument-order variant if it exists. PostgreSQL allows
-- overloaded functions, but keeping two nearly-identical RPCs makes the
-- schema cache and client behavior unnecessarily ambiguous.
drop function if exists public.add_order_item_online(uuid, text, integer, text);
drop function if exists public.add_order_item_online(text, uuid, text, integer);

create function public.add_order_item_online(
  p_device_id text,
  p_order_id uuid,
...
grant execute on function public.add_order_item_online(text, uuid, text, integer) to authenticated;
