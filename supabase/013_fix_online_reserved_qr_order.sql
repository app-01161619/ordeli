-- 013_fix_online_reserved_qr_order.sql
-- Fix online order creation when a QR is reserved to a different seller device.
-- Online, the authenticated seller may assign any QR belonging to that seller.
-- Device-specific reservation ownership is enforced by sync_offline_order for offline-created orders.
...
begin
  ...
  -- A QR reservation is only an offline safety mechanism. While online, the authenticated seller may assign any QR that belongs to this seller.
